...
export default {
  ...
  methods: {
    ...
    sendOtp() {
      axios
        .post("auth/get-otp", { email: this.email_register })
        .then((response) => {
          this.wait_until = response.data.timestamp;
          this.countDown();
          this.showAlert(
            "Gửi mã OTP thành công",
            "Gửi mã OTP thành công, vui lòng kiểm tra Email của bạn !",
            "success",
            ""
          );
        })
    ...
    convertSeconds(seconds) {
      ...
      return (
        (result_minutes >= 10 ? result_minutes : "0" + result_minutes) +
        ":" +
        (result_seconds >= 10 ? result_seconds : "0" + result_seconds)
      );
    },
  },
};
